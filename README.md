...
`setup.sh` asks for your vault path and symlinks skills into this repository's local agent directories. It does not write global config or global skill links by default. To preview changes, run `bash setup.sh --dry-run`. To also install global config and skill symlinks for use from any project, run `bash setup.sh --global-skills`.
...
Codex reads the `AGENTS.md` file at the repo root for project context. `setup.sh --global-skills` installs skills globally to `~/.codex/skills/`, making them available from any project. Either:

- Run `setup.sh --global-skills` to create symlinks globally, OR
...
Gemini agents read `GEMINI.md` at the repo root. `setup.sh --global-skills` installs skills globally to `~/.gemini/antigravity/skills/`, making them available from any project. Either:
...
- Run `setup.sh --global-skills` to create global symlinks, OR
...
OpenClaw is a local agent daemon that exposes itself through chat channels (Telegram, Slack, Discord, etc.). It reads `AGENTS.md` first (priority 10 in its bootstrap chain), then discovers skills from two paths: `~/.openclaw/skills/` (managed) and `~/.agents/skills/` (shared). Both are wired by `setup.sh --global-skills`.

Skills are auto-registered as slash commands — `/wiki-ingest`, `/wiki-query`, `/wiki-history-ingest`, etc. all work natively.

- Run `setup.sh --global-skills` to create symlinks in both paths, OR
...
When `OBSIDIAN_VAULT_PATH` points to Alex's Codex memory vault, the generic
standalone paths in this README are not copied directly. `wiki-write-guard`
translates or blocks writes so routine reports and staging can run unattended
without creating unmanaged root schemas.
...
| `wiki-write-guard`      | Check wiki writes before they run                 | `/wiki-write-guard`      |
...
│
├── ~/.gemini/antigravity/skills/  → optional global symlinks (setup.sh --global-skills)
├── ~/.codex/skills/               → optional global symlinks (setup.sh --global-skills)
├── ~/.hermes/skills/              → optional global symlinks (setup.sh --global-skills)
├── ~/.openclaw/skills/            → optional global symlinks (setup.sh --global-skills)
├── ~/.agents/skills/              → optional global symlinks (setup.sh --global-skills)
│
...
The whole point is that your wiki should stay up to date as you work across different codebases. You don't want to come back to the obsidian-wiki repo every time. For that mode, run `bash setup.sh --global-skills` to install global skills that work from any project.

When you run `bash setup.sh`, it configures this repository only:

1. It uses `.env` for the vault path.
2. It symlinks `.skills/*` into this repository's local `.claude`, `.cursor`, `.windsurf`, and `.agents` discovery directories.

When you run `bash setup.sh --global-skills`, it additionally:

1. Writes `~/.obsidian-wiki/config` with your vault path and repo location.
2. Symlinks `wiki-update` and `wiki-query` into `~/.claude/skills/` so they're available everywhere.
3. Symlinks all skills into `~/.gemini/antigravity/skills/` for global Gemini/Antigravity access.
4. Symlinks all skills into `~/.codex/skills/` for global Codex access.
5. Symlinks all skills into `~/.hermes/skills/` for global Hermes access.
6. Symlinks all skills into `~/.openclaw/skills/` and `~/.agents/skills/` for OpenClaw and other AGENTS.md-aware agents.
...
3. Run `bash setup.sh` to symlink into this repository's local agent directories
4. Optionally run `bash setup.sh --global-skills` to make skills available from other projects
